...
class AbstractGraphEditDistance(object):
    ...
    def print_matrix(self):
        print("Cost matrix:")
        cost_matrix = self.create_cost_matrix()  # 假设 create_cost_matrix 返回一个 2D 列表或 numpy 矩阵
        for column in cost_matrix:
            row_str = ""  # 用于存储每一行的内容
            for row in column:
                if row == sys.maxsize:
                    row_str += "inf\t"  # 如果是无穷大，用 "inf" 表示
                else:
                    row_str += "%.2f\t" % float(row)  # 否则保留两位小数
            print(row_str.strip())  # 输出当前行，去除末尾的多余空白字符
