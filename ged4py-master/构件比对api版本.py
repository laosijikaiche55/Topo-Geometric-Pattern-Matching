...
class Face:
    ...
    def compute_metric_tensor(self):
        """
        通过控制点计算度量张量（Metric Tensor）。
        度量张量定义为所有相邻单位向量的外积之和（Gram 矩阵）。

        返回:
            np.ndarray: 2x2 的度量张量矩阵，保留5位小数。
        """
        if len(self.control_points) < 2:
            raise ValueError("至少需要2个控制点来计算度量张量")

        # 计算所有相邻单位向量
        unit_vectors = []
        n = len(self.control_points)
        for i in range(n):
            # 从点 i-1 指向点 i（自动处理环形闭合）
            v = self.unit_vector(self.control_points[i - 1], self.control_points[i])
            unit_vectors.append(v)

        # 堆叠为矩阵并计算 Gram 矩阵
        tensor = np.array(unit_vectors)  # shape: (n, 2)
        metric_tensor = np.round(tensor.T @ tensor, 5)  # shape: (2, 2)
...
def compare_graph2(ifc_file_path1):
    ...
    output_file_path = r"E:\原电脑d盘新\BIM模型\BIM\中建壹品汉芯公馆\待对比构件\压缩副本.ifc"
...
def compare_graph1(ifc_file_path1):
    ...
    element= ifc_file.by_id("0KwBo$Cpv8NuIONhmC8UYn")
...
# IFC 文件路径
# ifc_file_path1 = r"E:\原电脑d盘新\BIM模型\单元实验\简单几何体\立方体微小扰动0.1mm.ifc"
# ifc_file_path2 = r"D:\BIM模型\BIM\中建壹品汉芯公馆\pkpm导出\半标准层.ifc"

ifc_file_path1 = r"C:\Users\admin\Desktop\蔡建华毕业论文\墙1.ifc"
...
compare_graph1(ifc_file_path1)


app = Flask(__name__)


@app.route('/api/compare-components', methods=['POST'])
def compare_components():
    ...
    if not data or 'guid' not in data:
        return jsonify({
            "status": "error",
            "message": "Missing 'guid' in request body"
        }), 400

    target_guid = data['guid']

    # 简单的GUID格式验证 (可根据实际需求调整)
    if not isinstance(target_guid, str) or len(target_guid) < 10:
        return jsonify({
            "status": "error",
    ...
    try:
        # 调用原有比对函数
        matching_guids = compare_graph1(ifc_file_path1)

        # 返回结果
        return jsonify({
            "status": "success",
            "matching_guids": matching_guids,
            "count": len(matching_guids)z
        })
